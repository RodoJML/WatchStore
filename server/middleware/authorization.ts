...
function parseAuthorizationHeader(req: Request, res: Response, next: NextFunction) {
    const token = req.headers.authorization?.split(' ')[1];

    if (token) {
        users.verifyTokenAsync(token)
            .then(
                (user: any) => { req.body.user = user; next(); }) // Check this function later!!! 
            .catch(
                (err: any) => { next({ code: 401, message: err }); });
    }
    else {
        next();
    }
